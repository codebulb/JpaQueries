...
public class ProductServiceWithNamedNativeQueries extends ProductService {
    CustomerService customerService;
    
    public List<Product> findForPurchaseCustomerPremium() {
        return em.createNamedQuery(Product.SQL_FIND_FOR_PURCHASE_CUSTOMER_PREMIUM, Product.class)
                .getResultList();
    }
    
    public Map<Customer, Double> sumPriceByPurchaseCustomer() {
        List results = em.createNamedQuery(Product.SQL_SUM_PRICE_BY_PURCHASE_CUSTOMER)
                .getResultList();
